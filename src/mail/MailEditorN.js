...
import type {Recipients, ResponseMailParameters} from "./SendMailModel"
...
import {
	conversationTypeString, createInlineImage,
	getDefaultSender,
	getEnabledMailAddressesWithUser, getSupportMailSignature,
	parseMailtoUrl,
...
export function newMailEditorAsResponse(args: ResponseMailParameters, mailboxDetails: ?MailboxDetail, blockExternalContent: boolean, inlineImages: ?Promise<InlineImages>): Promise<Dialog> {
	const init = (mailbox) => {
		return defaultSendMailModel(mailbox).initAsResponse(args).then(model => {
			return _createMailEditorDialog(model, blockExternalContent, inlineImages || undefined)
		})
	}
